...
2. 判断适用范围：
   - **项目专用**：写入当前项目的 `docs/` 子目录或 `.pi/agent/skills/`，并按项目约定维护索引。
   - **通用 skill/note**：若用户未明确指定目标位置，询问希望存放到哪个 skill 仓库或 notes 目录。
...
- **历史会话**：用 `find . -maxdepth 1 -type f -printf '%T@ %f\n' | sort -nr` 定位当前目录 `.jsonl`；若项目使用 Trellis，同时检查 `.trellis/workspace/` 和 `.trellis/tasks/`；先抽 `user/assistant` 摘要，避免整读。
- **指定材料**：优先使用用户给出的文件、目录、issue、PRD 或任务上下文，不要按默认路径猜测覆盖。
...
### 3. 项目专用文档：写入 `docs/` 并按项目约定维护索引

当产物只对当前项目有效时：

1. 选择或创建合适的 `docs/<topic>/` 子目录（如 `protocol/`、`models/`、`desktop/`、`operations/`）。
2. 写入 Markdown，保持简短、可执行、不过度嵌入一次性细节。
3. 若项目已有 Documentation index，在 `AGENTS.md` 或约定索引中新增一行：
   - 说明查看时机
   - 指向新增文档路径
4. 若 `AGENTS.md` 由工具托管或没有索引区，不要硬改托管块；改为按项目约定更新 README、docs index，或先询问用户。
5. 如果相关术语也出现在 `CONTEXT.md`，在对应条目中添加反向链接。
...
1. 先在目标 skills 目录查重。
2. 能力相同则更新；能力不同则新建。
3. 若用户未明确指定目标位置，**必须询问用户**希望存放到哪个位置（如 `~/.agents/skills/`、`zine-skills`、项目级 `.pi/agent/skills/`）。
4. 写入 `SKILL.md`：frontmatter + Quick start + Workflow + Lessons learned + Checklist。

### 5. 校验

- [ ] 路径正确，文件非空。
- [ ] 项目文档已按项目约定更新索引；若未更新，已说明原因。
- [ ] 若存在 `CONTEXT.md`，术语与分类文档无冲突，必要时互相链接。
...
- `AGENTS.md` 适合作为轻量索引，但若它由工具托管或没有索引区，不要强行改写。
- 若项目存在术语表（如 `CONTEXT.md`），分类文档应与术语表互相引用，避免未来 agent 只读其一。
